...
//localStorage.js에서 localStorage.getItem를 불러옴
...
function App() {
  ...
  /** components파일 안에 localStorage.js 컴포넌트를 만들어 localStorage메서드에 대한 코드중복을 피했습니다. */

  /**getLocalStorage는 localStorage의 getItem메소드를 사용한 것입니다.
   * 서버한테 로그인 성공시 받아오는 username를 "name"키값으로 localStorage에 담았습니다.
   */
  const name = getLocalStorage("name");
  return (
    <>
      {/*리액트 라우터 설정을 해서 각 페이지에 url를 만들고, 위에 헤더 부분은 레이아웃으로 계속 있었으면 해서 Layout컴포넌트는 전체적으로 
      ...
      {/* 로그인 유지를 위해 username를 사용했고, 
        username이 있다면 Logout컴포넌트를,
